...
use crate::descriptor::{ShippingLaneDescriptor, ShippingLaneNetwork};
...
fn push_parts<P: HasXY>(roads: &mut Vec<ShippingLaneDescriptor>, parts: &[Vec<P>]) {
    for part in parts {
        if part.len() < 2 {
            continue;
        }

        roads.push(ShippingLaneDescriptor {
...
pub fn run_preprocessor() -> Result<(), Error> {
    ...
    let network = ShippingLaneNetwork(roads);
