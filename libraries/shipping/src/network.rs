...
use shipping_lanes::index::{shape_is_spherical, tile_xy};
...
fn locate(
    ...
    let world = shape.position_unit_to_local(unit, 0.0);
...
impl ChainBuilder {
    ...
    fn flush(&mut self, chains: &mut Vec<ShippingChain>, shape: TerrainShape, spherical: bool) {
...
pub fn chains_for_tile(
    tile: TileCoordinate,
    network: &shipping_lanes::descriptor::ShippingLaneNetwork,
    candidates: &[u32],
    shape: TerrainShape,
) -> Vec<ShippingChain> {
    ...
    for &road_index in candidates {
        ...
        let located = locate(lon0, lat0, shape, spherical, tile, tile_count);
        builder.push(&located, &mut chains, shape, spherical);

        for &(lon1, lat1) in &road.points[1..] {
            let located0 = locate(lon0, lat0, shape, spherical, tile, tile_count);
            let located1 = locate(lon1, lat1, shape, spherical, tile, tile_count);
            ...
            for step in 1..=steps {
                ...
                let located = locate(lon, lat, shape, spherical, tile, tile_count);
                builder.push(&located, &mut chains, shape, spherical);
            }
