...
/// this is deliberately the only whole-globe structure any road-aware system (`automobiles`,
...
/// counterpart to `automobiles`' fine per-tile chain building. Cell `(ix, iy)` uses the exact same
...
///
/// Unlike `automobiles`' path-following use case (which needs precise, elevation-aware waypoints),
